    ...
    it('createFunction', function () {
        return client.createFunction(body)
            .then(function (response) {
                ...
                debug("createFunction response (%j)", response.body);
                return client.invocations(body.FunctionName, invoke_body, invoke_options)
            }).then(function (response) {
                debug("invocations response (%s)", response.body);
                invoke_options.logToBody = "false";
                invoke_options.logType = "None";
                return client.invocations(body.FunctionName, invoke_body, invoke_options)
            }).then(function (response) {
                debug("invocations response (%s)", response.body);
                ...
                return client.invocations(brn, invoke_body, invoke_options)
            }).then(function (response) {
                debug("invocations FunctionBrn (%s)", response.body);
                return client.createAlias(body.FunctionName, alias_body)
            }).then(function (response) {
                ...
                alias_body.Description = "change desc";
                return client.updateAlias(response.body.FunctionName, response.body.Name, alias_body)
            }).then(function (response) {
                ...
                return client.deleteAlias(body.FunctionName, alias_body.Name)
            }).then(function (response) {
